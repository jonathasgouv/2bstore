type ClusterBy = keyof Installment
...
export function clusterInstallments(
  installmentsList: Installment[],
  clusterBy: keyof Installment
) {
  const clusteredInstallments: Record<string, Installment[]> = {}
...
function pickMaxOptionCount(
  clusteredInstallments: Record<string, Installment[]>,
  clusterBy: ClusterBy
) {
